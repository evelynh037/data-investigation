...
•Third step: by merging two dataframe(information about each recipes and users reviews on the recipes), one recipe can appear in many rows if more than one users leave review for it(each reviews for the same recipe would take one line). Also, to explore the question we have, we just need infomation about the recipe and also average rating. Since we already calculated the average rating and assign to each rows(rows of identical recipe would have the same average rating), we decided to drop duplicates based on the "id" column which stores the unique number for individual recipe. By doing this, we can use count to perform data analysis without being influence by having one recipe appear more than others in the dataframe and eliminate potential bias.<br />
